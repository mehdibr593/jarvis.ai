...
def ask_ai(prompt, gemini_func=None):
    global OFFLINE_MODE
...
def ask_ai_smart(prompt, gemini_func=None):
    """
    Smart router:
    - OFFLINE_MODE = force local
    - no internet = local fallback
    - otherwise Gemini if available
    """
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are J.A.R.V.I.S, the personal AI assistant of Master Mehdi Barchichou. "
...
class JarvisLive:
    ...
    def speak_error(self, tool_name: str, error: str):
        ...
        self.speak(f"Master Mehdi, {tool_name} encountered an issue. {short}")
    ...
    async def _listen_audio(self):
        if not SOUNDDEVICE_AVAILABLE:
            print("[JARVIS] 🎤 Mic disabled (sounddevice not available)")
            return
    ...
    async def _play_audio(self):
        if not SOUNDDEVICE_AVAILABLE:
            print("[JARVIS] 🔊 Audio output disabled (sounddevice not available)")
            # Consume audio data to prevent queue buildup
            while True:
                try:
...
# ===== JARVIS OFFLINE COMMAND SYSTEM =====
OFFLINE_MODE = False

def handle_jarvis_commands(user_text):
    global OFFLINE_MODE
    t = user_text.lower().strip()

    if "jarvis go offline" in t:
        OFFLINE_MODE = True
        return "Going offline, Master."

    if "jarvis go online" in t:
        OFFLINE_MODE = False
        return "Back online, Master."

    return None


def smart_ai_router(prompt, gemini_func=None):
    if OFFLINE_MODE:
        return ask_ollama(prompt)
